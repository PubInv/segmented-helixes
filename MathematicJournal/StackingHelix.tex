...
%% Added for the purpose of minor revisions
...
\Author{Robert L. Read $^{1,\dagger,\ddagger}$\orcidA{0000-0003-4749-7045}}
...
\secondnote{This paper is an extended version of our paper In Proceedings of the IMA Conference on Mathematics of Robotics. Springer, 2020, pp. 115–124.}
...
This paper proves Lord's Observation.
...
previous work \cite{kahn1989defining} are combined with
...
We prove that any subunit can produce a toroid-like helix or a maximally-extended helix, forming a continuous spectrum based
on joint-face normal twist.
This software, website and paper, taken together, compute, render, and
catalog an exhaustive ``zoo'' of 28 uniquely-shaped platonic helices,
such as the Boerdijk-Coxeter tetrahelix and various
species of helices formed from dodecahedra.
}

% Keywords
\keyword{solid geometry, helix, Chasles' theorem, platonic helix, tetrahelix,
  linear alegebra,
%%  \change[RLR]{linear alegebra}{linear algebra},
  computer graphics}
...
of the helix.
However, the fact that it is helical in nature is unsurprising, since it follows directly from Observation~[O\ref{obs:lords} and the
fact that the strands of DNA are formed of repeated chemical units of approximately the same shape arranged sequentially.
...
Becuase $\overline{BC}$ is a bisector of $\angle B'BA$, $\angle ABC = \frac{\pi - \theta}{2}$. Therefore $\angle ACB = \pi - 2(\pi-\theta)/2 = \theta$.
...
It is a screen shot taken from the realtime interactive website \cite{segmentedhelixinteractive}.
...
of the helix and the $z$ axis, and therefore the angle of any one segment against the helix axis.
...
\begin{Theorem}[Segmented Helix]
  ...
  constructed from object $i$, a {\em periodic chain.}
...
\begin{figure}[H]
  \centering
  \captionsetup{justification=centering}
     \includegraphics[width=13.5 cm]{figures/TwistSpectrumGeneral.png}
...
\begin{proof}
  ...
  prism in balanced position. The $B$ joint is on the far-side face of the
  prism in the figure. As usual, $\tau$ is measured against the
  ...
  an angle $\delta$ around negative $z$-axis, and the $\overrightarrow{N_C}$ face normal
  an angle $\gamma$ around the positive $z$-axis.


  Because this theorem is about finding a particular $\tau$,
  ...
  $2\pi$. The angles $\alpha(\tau$ and $\beta(\tau)$ are measured in the $XY$ plane against the positive $X$ axis.

    Let $\gamma$ and $\delta$ be the angles in between the $A$ face-vector and
    $B$ face-vectors respectively formed with the
    $\overrightarrow{BA}$ vector and $\overrightarrow{AB}$ respectively.
    Unlike $\alpha(\tau)$ and $\beta(\tau)$, $\gamma$ and $\delta$ are
    not confined to a plane, and describe a cone-like object as $\tau$ varies.
    ...
    Without loss of generality, let $\alpha(\tau)$ have a range
    including $0$ and $2\pi$.

    Both $\alpha$ and $\beta$ are continuous functions,
    ...
    by the Intermediate Value Theorem of real analysis, there is a $\tau$ where
    ...
    \begin{figure}[H]
      \centering
  \captionsetup{justification=centering}
     \includegraphics[width=13.5 cm]{figures/TwistSpectrumEqual.png}
...
\unskip
    ...
    the face about the face normal so that they cancel completely. By Theorem~\ref{thm:segmenthelixtest}, this must occur at the same time on both sides,
    ...
    is a straight line of a degenerate torus.
        \begin{figure}[H]
          \centering
  \captionsetup{justification=centering}
     \includegraphics[width=13.5 cm]{figures/TwistSpectrumProjection.png}
...
\begin{table}[H]
  ...
  \caption{The Platonic Helices} % title of Table
	\begin{adjustwidth}{-\extralength}{0cm}
          % \begin{tabularx}{l l r r r r r r r}
          \newcolumntype{C}{>{\centering\arraybackslash}X}
	  \newcolumntype{L}{>{\arraybackslash}X}
\newcolumntype{b}{L}
\newcolumntype{s}{>{\hsize=.5\hsize}C}
\newcolumntype{q}{>{\hsize=.4\hsize}C}

% \begin{tabularx}{\fulllength}{LLCCCCCCC}
\begin{tabularx}{\fulllength}{bqqqsssss}
  \toprule
  Name & Solid & \# of Analogs & C-Face \# & $ \tau $ & radius & $ \theta $ & d & $ \phi $ \\ [0.5ex] % inserts table
%heading
...
\acknowledgments{Thanks to Prof. Eric Lord for his direct communication.
...
{\textbf{Robert L. Read, PhD} is a computer scientist who was a U.S. Presidential Innovation Fellow in 2013. He is the founder of the charity Public Invention. His research interests include tetrahedral robots,
  biomedical engineering of human respiration devices, and ferrofluid pumping. Rob speaks Esperanto fluently.}
